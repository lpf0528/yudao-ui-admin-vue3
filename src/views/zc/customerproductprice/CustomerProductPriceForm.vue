<template>
  <Dialog :title="dialogTitle" v-model="dialogVisible">
    ...
    >
      ...
      <el-form-item label="授权价格" prop="authorizedPrice">
        <el-input v-model="formData.authorizedPrice" placeholder="请输入授权价格" />
      </el-form-item>
    </el-form>
    <template #footer>
...
/** 客户产品销售授权价 表单 */
defineOptions({ name: 'CustomerProductPriceForm' })
...
const formRules = reactive({
  ...
  authorizedPrice: [{ required: true, message: '授权价格不能为空', trigger: 'blur' }]
})
...
const open = async (type: string, id?: number) => {
  ...
  if (id) {
    ...
    try {
      formData.value = await CustomerProductPriceApi.getCustomerProductPrice(id)
    } finally {
...
const submitForm = async () => {
  ...
  formLoading.value = true
  try {
    const data = formData.value as unknown as CustomerProductPrice
