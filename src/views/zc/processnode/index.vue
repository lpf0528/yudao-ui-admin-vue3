<template>
    ...
    >
      ...
      <el-form-item>
        ...
        <el-button
          ...
          v-hasPermi="['zc:process-node:create']"
        >
        ...
        <el-button
          ...
          v-hasPermi="['zc:process-node:export']"
        >
  ...
  <!-- 表单弹窗：添加/修改 -->
  <ProcessNodeForm ref="formRef" @success="getList" />
</template>
...
const list = ref<ProcessNode[]>([]) // 列表的数据
...
const getList = async () => {
  loading.value = true
  try {
    const data = await ProcessNodeApi.getProcessNodePage(queryParams)
...
const handleDelete = async (id: number) => {
  try {
    ...
    // 发起删除
    await ProcessNodeApi.deleteProcessNode(id)
...
const handleDeleteBatch = async () => {
  try {
    // 删除的二次确认
    await message.delConfirm()
    await ProcessNodeApi.deleteProcessNodeList(checkedIds.value);
    checkedIds.value = [];
    message.success(t('common.delSuccess'))
    await getList();
  } catch {}
...
const handleRowCheckboxChange = (records: ProcessNode[]) => {
...
const handleExport = async () => {
  try {
    ...
    const data = await ProcessNodeApi.exportProcessNode(queryParams)
    download.excel(data, '工序节点配置.xls')
  } catch {
  } finally {
